...
export default async function PostPage({
  ...
  return (
    <div className="max-w-3xl mx-auto px-4 py-8">
      <Link href="/forum" className="inline-flex items-center gap-1 text-sm text-gray-400 hover:text-indigo-300 transition mb-4">
      ...
      <div className="bg-white/8 backdrop-blur-sm rounded-2xl border border-white/10 p-6">
        ...
        <div className="flex items-center gap-3 mt-4">
          <div className="w-10 h-10 bg-gradient-to-br from-indigo-400 to-purple-500 rounded-full flex items-center justify-center">
            <span className="text-white font-bold">{post.author.name.charAt(0).toUpperCase()}</span>
          </div>
          <div>
            <p className="font-medium text-gray-200">{post.author.name}</p>
            <p className="text-xs text-gray-500">
              {post.createdAt.toLocaleString("zh-CN", { year: "numeric", month: "long", day: "numeric", hour: "2-digit", minute: "2-digit" })}
            </p>
          </div>
        </div>
